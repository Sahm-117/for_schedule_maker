-- Allow an announcement to target a single person: a support/admin via
-- targetUserId (User FK) or a single participant via targetParticipantId
-- (Participant FK). Exactly one is set when a person is picked in the
-- composer; both null = whole audience. Nullable, idempotent, backward
-- compatible with the live app.

ALTER TABLE "Announcement" ADD COLUMN IF NOT EXISTS "targetUserId" UUID REFERENCES public."User"(id) ON DELETE SET NULL;
ALTER TABLE "Announcement" ADD COLUMN IF NOT EXISTS "targetParticipantId" UUID REFERENCES public."Participant"(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_announcement_target_user ON "Announcement"("targetUserId");
CREATE INDEX IF NOT EXISTS idx_announcement_target_participant ON "Announcement"("targetParticipantId");

-- participant_home: gate the pinned home-screen announcement so a
-- participant-targeted send only appears to that one participant, and a
-- support-targeted send never appears to any participant. Same body as
-- 20260923000000_sunday_attendance_late_rule.sql's participant_home, with
-- two extra conditions added to the 'announcement' subquery's WHERE clause.
