-- Participant profile: participants can add details the registration form does
-- not collect: date of birth and occupation (occupation was only captured for
-- leads registered by a support). They can also correct their email.
...
-- ── Participant: home (profile gains gender, age range, date of birth, occupation) ──
...
-- ── Participant: update their own details ────────────────────────────────────

CREATE OR REPLACE FUNCTION public.update_participant_profile(p_token TEXT, p_email TEXT, p_occupation TEXT, p_date_of_birth DATE)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, extensions
...
GRANT EXECUTE ON FUNCTION public.update_participant_profile(TEXT, TEXT, TEXT, DATE) TO anon, authenticated;
