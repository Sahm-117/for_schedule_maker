-- Hotfix for 20260925080000: participant_home read ParticipantWrapUp.id,
-- which doesn't exist (the table's key is participantId + cohortId), so the
-- participant Home failed to load for everyone. Same function, one line fixed.
