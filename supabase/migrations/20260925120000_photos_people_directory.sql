-- Photos & people directory.
-- 1. participant_home: the group's support photo ('group'.supportAvatarUrl) and
--    group members' photos ('members'[].avatarUrl). Otherwise identical to
--    20260925100000_scriptures_on_off.sql.
-- 2. participant_people(p_token): the participant's People page -- every active
--    support/admin linked to their cohort (UserCohort) with photo, name and
--    role, flagging their own support, plus their own group members (photo and
--    name only). Never returns another participant's phone number.
--
