...
-- next upcoming cohort's name + start date, the Sunday class start time, the
-- admin-uploaded landing photos, and now the admin-editable landing copy
-- (Settings > Website — see frontend/src/components/landing/landingContent.ts
-- and AdminWebsitePage.tsx). AppSetting rows are staff-only under RLS, so
-- this is a SECURITY DEFINER function exposing only those fields — never a
-- broader grant on AppSetting/Cohort themselves.
--
...
--
-- This CREATE OR REPLACEs the function from 20260926170000_public_fof_landing.sql,
-- unchanged except for the added `landingContent` field.
...
BEGIN
  ...
  -- Admin-editable landing page copy (Settings > Website). Absent/partial
  -- is fine — the frontend deep-merges this against its own defaults.
  SELECT value INTO v_landing_content
  FROM public."AppSetting"
  WHERE "settingKey" = 'landing_content';
  ...
  RETURN json_build_object(
    ...
    'landingContent', COALESCE(v_landing_content, '{}'::jsonb)
  );
END;
$function$;
